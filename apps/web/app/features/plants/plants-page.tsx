...
export function PlantsPage({
  ...
  async function decommissionPlant(plant: Plant) {
    if (
      !window.confirm(
        `ปิดใช้งานโรงไฟฟ้า “${plant.name}”? ข้อมูลเดิมจะยังอยู่ แต่ Middleware จะส่งข้อมูลเข้า Plant นี้ไม่ได้`,
      )
    )
      return;
  ...
  async function hardDeletePlant(plant: Plant) {
    ...
    if (
      window.prompt(
        `คำสั่งนี้จะลบ Plant, Device, Metadata และ normalized telemetry ถาวร\nพิมพ์ ${expected}`,
      ) !== expected
    )
      return;
  ...
  async function exportAllCSV() {
    ...
    if (!response.ok) {
      toast.error("ดาวน์โหลด CSV ไม่สำเร็จ");
      return;
    }
  ...
  async function exportOneCSV(plant: Plant) {
    const response = await api(
      `/api/v1/plants/${encodeURIComponent(plant.id)}/export`,
    );
    if (!response.ok) {
      toast.error("ดาวน์โหลด CSV ไม่สำเร็จ");
      return;
  ...
  async function importPlantCSVFile(file: File) {
    ...
    try {
      const response = await api("/api/v1/plants/import", {
        method: "POST",
        headers: { "X-CSRF-Token": csrfToken() },
        body: formData,
      });
      if (!response.ok)
        throw new Error("Import CSV ไม่สำเร็จ — ตรวจรูปแบบไฟล์");
      ...
      toast.success(
        `Import สำเร็จ: Plant ใหม่ ${result.plantsCreated}, Plant อัปเดต ${result.plantsUpdated}, Device ใหม่ ${result.devicesCreated}, Device อัปเดต ${result.devicesUpdated}` +
          (result.rowsSkipped > 0
            ? `, ข้าม ${result.rowsSkipped} แถว (${(result.errors ?? []).slice(0, 2).join("; ")})`
            : ""),
      );
      await loadPlants();
    } catch (cause) {
  ...
  const filteredPlants =
    lifecycleFilter === "ALL"
      ? visiblePlants
      : visiblePlants.filter(
          (plant) => plant.lifecycleStatus === lifecycleFilter,
        );
  ...
  return (
    <div className="content plants-content">
      <div className="section-heading">
        ...
        <div className="heading-actions">
          <Button
            ...
            title="รีเฟรช"
            aria-label="รีเฟรชรายการโรงไฟฟ้า"
          >
            <RefreshCw size={18} />
          </Button>
          ...
          <Button
            variant="secondary"
            compact
            onClick={() => void downloadPlantTemplate()}
            title="ดาวน์โหลด Template เปล่า"
          >
            Template
          </Button>
      ...
      <Select value={lifecycleFilter} onValueChange={setLifecycleFilter}>
        <SelectTrigger className="min-w-44" aria-label="กรอง Lifecycle">
          <SelectValue />
        </SelectTrigger>
        <SelectContent>
          ...
          emptyMessage={
            <div className="table-state">
              {error
                ? ""
                : plants.length === 0
                  ? "ยังไม่มีโรงไฟฟ้าในขอบเขตที่คุณเข้าถึงได้"
                  : "ไม่พบโรงไฟฟ้าที่ตรงกับตัวกรอง"}
            </div>
          }
        >
          <TableColumn
            ...
            body={(plant: Plant) => (
              <div className="grid gap-1">
                ...
                <small className="flex items-center gap-1 text-[11px] text-ink-soft">
                  <MapPin size={13} /> {plant.code} · {plant.timezone}
                </small>
              </div>
            )}
          />
          <TableColumn
            ...
            body={(plant: Plant) => (
              <div className="grid gap-1">
                <span>{plant.organizationName}</span>
                <small className="block text-[11px] text-ink-soft">
          ...
          <TableColumn
            ...
            body={(plant: Plant) => (
              <div className="grid gap-1">
                <span>
                  {plant.installedDcKw == null
                    ? "-"
                    : `${plant.installedDcKw.toLocaleString()} kWdc`}
                </span>
                <small className="block text-[11px] text-ink-soft">
                  {plant.installedAcKw == null
                    ? "ไม่ระบุ AC"
                    : `${plant.installedAcKw.toLocaleString()} kWac`}
                </small>
              </div>
            )}
          />
              ...
              <StatusTag
                tone={
                  plant.lifecycleStatus === "OPERATIONAL"
                    ? "active"
                    : plant.lifecycleStatus === "OFFLINE"
                      ? "offline"
                      : "degraded"
                }
              >
                {plant.lifecycleStatus.replace("_", " ")}
              </StatusTag>
            )}
          />
          <TableColumn
            ...
            sortable
            body={(plant: Plant) => (
              <StatusTag tone={plant.isActive ? "active" : "revoked"}>
                {plant.isActive ? "ใช้งาน" : "ปิดใช้งาน"}
              </StatusTag>
            )}
          />
          <TableColumn
            ...
            body={(plant: Plant) => (
              <div className="row-actions">
                <Button
                  ...
                  title="จัดการ Device"
                  aria-label={`จัดการ Device ใน ${plant.name}`}
                >
                  <Cpu size={17} />
                </Button>
                <Button
                  variant="icon"
                  onClick={() => void exportOneCSV(plant)}
                  title="Export CSV"
                  aria-label={`Export CSV ของ ${plant.name}`}
                >
                  <Download size={17} />
                </Button>
                {can(user, "plant", "update") && <Button
                ...
                {plant.isActive && can(user, "plant", "update") && (
                  <Button
                    ...
                    title="ปิดใช้งาน"
                    aria-label={`ปิดใช้งาน ${plant.name}`}
                  >
                    <ArchiveX size={17} />
                  </Button>
                )}
...
function DeviceManagement({
  ...
  const [testOutcomes, setTestOutcomes] = useState<
    Record<string, { pending: boolean }>
  >({});
  const [testReadResult, setTestReadResult] = useState<{
    device: Device;
  ...
  async function decommissionDevice(device: Device) {
    if (
      !window.confirm(
        `ปิดใช้งาน Device “${device.name}”? ข้อมูล telemetry เดิมจะยังคงอยู่`,
      )
    )
      return;
    const response = await api(
  ...
  async function hardDeleteDevice(device: Device) {
    ...
    if (
      window.prompt(
        `คำสั่งนี้จะลบ Device, Metadata และ normalized telemetry ถาวร\nพิมพ์ ${expected}`,
      ) !== expected
    )
      return;
    const response = await api(
  ...
  async function runCommand(
    kind: "test-connection" | "test-read",
    device: Device,
  ) {
    ...
    try {
      ...
      const data = (await response.json()) as {
        ...
        data?: {
          reading?: {
            registerAddressMap?: Record<string, number>;
            collectTime?: number;
          };
        };
      };
      ...
      setTestReadResult({
        device,
        dataItemMap,
        collectTime: data.data?.reading?.collectTime,
      });
      toast.success(
        count > 0
          ? `อ่านค่าสำเร็จ (${count} ค่า)`
          : "อ่านค่าสำเร็จ แต่ไม่มีค่า Register กลับมา — เช็ก Register Metadata ของ Device Model นี้",
      );
    } catch (cause) {
  ...
  const lastObservedAt = latestReadings.reduce<string | undefined>(
    (latest, reading) =>
    ...
    <div className="content devices-content">
      <div className="section-heading">
        <div className="registry-title">
          <Button
            variant="icon"
            onClick={onBack}
            title="กลับไปโรงไฟฟ้า"
            aria-label="กลับไปโรงไฟฟ้า"
          >
            <ArrowLeft size={18} />
          </Button>
          <div>
            <p>{plant.code}</p>
            <h2>Device ใน {plant.name}</h2>
          </div>
        </div>
        <div className="row-actions">
          <span className={`live-chip ${liveState}`}>
            <Wifi size={15} />
            {liveState === "connected"
              ? "Live data"
          ...
          <Button
            ...
            title="รีเฟรช"
            aria-label="รีเฟรชรายการ Device"
          >
            <RefreshCw size={18} />
          </Button>
      ...
      >
        <div className="bg-white p-4">
          <small className="font-bold text-slate-500">Installed capacity</small>
        ...
        <div className="bg-white p-4">
          <small className="font-bold text-slate-500">Devices</small>
          <strong className="mt-1 block text-xl text-slate-900">
        ...
        <div className="bg-white p-4">
          <small className="font-bold text-slate-500">Reporting</small>
          <strong className="mt-1 block text-xl text-slate-900">
            {latestReadings.length.toLocaleString()}
          </strong>
          <span className="text-xs text-slate-500">Device ที่มีค่าล่าสุด</span>
        </div>
        <div className="bg-white p-4">
          <small className="font-bold text-slate-500">Latest observed</small>
          <strong className="mt-1 block text-sm text-slate-900">
            {lastObservedAt ? formatDate(lastObservedAt) : "ยังไม่มีข้อมูล"}
          </strong>
          <span className="text-xs text-slate-500">
            Timezone: {plant.timezone}
          </span>
      ...
      {loading ? (
        ...
        >
          ...
          <TableColumn
            ...
            body={(device: Device) => (
              <div className="grid gap-1">
                <span>
                  {device.modbusHost
                    ? `${device.modbusHost}:${device.modbusPort}`
                    : "ไม่ใช่ Modbus device"}
                </span>
                <small className="block text-[11px] text-ink-soft">
                  {device.modbusHost
                    ? `unit ${device.modbusUnitId}`
                    : device.deviceType}
                </small>
              </div>
            )}
          ...
          <TableColumn
            ...
            sortable
            body={(device: Device) => (
              <StatusTag tone={device.isActive ? "active" : "revoked"}>
                {device.isActive ? "ใช้งาน" : "ปิดใช้งาน"}
              </StatusTag>
            )}
          />
          <TableColumn
              ...
              return (
                <div className="row-actions">
                  {canUpdateDevice && <Button
                    ...
                    title={
                      canTest ? "ทดสอบการเชื่อมต่อ" : "ต้องตั้งค่า IP/Port ก่อน"
                    }
                  ...
                  <Button
                    ...
                    title="ดูค่าล่าสุดทั้งหมด"
                    aria-label={`ดูค่าล่าสุดของ ${device.name}`}
                  >
                    <Eye size={17} />
                  </Button>
                  {canUpdateDevice && <Button
                  ...
                  {device.isActive && canUpdateDevice && (
                    <Button
                      ...
                      title="ปิดใช้งาน"
                      aria-label={`ปิดใช้งาน ${device.name}`}
                    >
                      <ArchiveX size={17} />
                    </Button>
                  )}
      ...
      {testReadResult && (
        <TestReadDialog
          ...
          onClose={() => setTestReadResult(null)}
        />
      )}
      {editor && (
        <DeviceEditor
          plant={plant}
          device={editor === "create" ? undefined : editor}
          onClose={() => setEditor(null)}
          onSaved={() => {
            setEditor(null);
            void loadDevices();
...
function DeviceDetailView({
  ...
  return (
    <div className="content device-detail-content">
      <div className="section-heading">
        <div className="registry-title">
          <Button
            ...
            onClick={onBack}
            title="กลับไป Device"
            aria-label="กลับไป Device"
          >
            <ArrowLeft size={18} />
          </Button>
          <div>
            <p>
              {plant.code} · {device.externalId}
              {reading ? ` · ${formatDate(reading.observedAt)}` : ""}
            </p>
            <h2>{device.name}</h2>
          </div>
        </div>
      ...
      <section
        className="grid gap-px overflow-hidden rounded-md border border-slate-200 bg-slate-200 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6"
        aria-label="ค่าปัจจุบัน (เฉพาะ parameter ที่เปิดใช้งาน)"
      >
        {values.length === 0 && (
          <div className="table-state col-span-full">
            ยังไม่มี telemetry (เฉพาะ parameter ที่เปิดใช้งาน) สำหรับ Device นี้
          </div>
        )}
        {values.map(([key, value]) => {
          ...
          return (
            <div className="bg-white p-2.5" key={key}>
              <small
                className="block truncate text-[11px] font-bold text-slate-500"
...
function TestReadDialog({
  ...
  const entries = Object.entries(result.dataItemMap).sort(([a], [b]) =>
    a.localeCompare(b),
  );
  return (
    <Dialog
    ...
    >
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <div>
            <DialogDescription>{result.device.externalId}</DialogDescription>
            <DialogTitle>ผลทดสอบอ่านค่า {result.device.name}</DialogTitle>
          </div>
        </DialogHeader>
        <DialogBody>
          {result.collectTime && (
            <div className="px-5 pt-4 text-sm">
              <small className="block text-slate-500">อ่านเมื่อ</small>
              <strong>
                {formatDate(new Date(result.collectTime).toISOString())}
              </strong>
            </div>
          )}
          <div className="max-h-[55vh] overflow-auto border-t border-slate-200 px-5 py-3">
            {entries.length === 0 ? (
              ...
              entries.map(([key, value]) => (
                <div
                  key={key}
                  className="flex items-center justify-between gap-4 border-b border-slate-100 py-2 text-sm last:border-0"
                >
...
function LatestValues({ reading }: { reading?: LatestTelemetry }) {
  if (!reading)
    return (
      <div>
        <span>-</span>
        <small>ยังไม่มี telemetry</small>
      </div>
    );
  ...
  return (
    <div
      ...
      title={`รับเมื่อ ${formatDate(reading.receivedAt)}`}
    >
      {values.map(([key, value]) => (
        <small key={key}>
      ...
      {reading.parameterCount > values.length && (
        <small>+{reading.parameterCount - values.length} parameters</small>
      )}
    </div>
  );
}
...
function DeviceEditor({
  ...
  const [modbusPort, setModbusPort] = useState(
    device?.modbusPort?.toString() ?? "502",
  );
  const [modbusUnitId, setModbusUnitId] = useState(
    device?.modbusUnitId?.toString() ?? "1",
  );
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const response = await api(
        device
          ? `/api/v1/plants/${plant.id}/devices/${device.id}`
          : `/api/v1/plants/${plant.id}/devices`,
        {
      ...
      if (response.status === 404)
        throw new Error("ไม่พบ Plant/Device หรือบัญชีนี้ไม่มีสิทธิ์");
      if (response.status === 409)
        throw new Error("External ID นี้มีอยู่แล้วใน Plant");
      if (!response.ok) {
        const detail = await response.text().catch(() => "");
        throw new Error(
          `บันทึกไม่ได้ (${response.status})${detail ? `: ${detail}` : ""}`,
        );
      }
      onSaved();
    } catch (cause) {
  ...
  return (
    ...
    >
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <div>
            <DialogDescription>{plant.code}</DialogDescription>
            <DialogTitle>
              {device ? "แก้ไข Device" : "เพิ่ม Device"}
            </DialogTitle>
          </div>
        </DialogHeader>
        <DialogBody>
          <form className="plant-editor-form" onSubmit={submit}>
            ...
            <label className="full-field">
              ...
              >
                <SelectTrigger>
                  <SelectValue placeholder="ยังไม่มี Device Model — สร้างที่หน้า Register Metadata ก่อน" />
                </SelectTrigger>
                <SelectContent>
                  {models.map((m) => (
                    <SelectItem key={m.id} value={m.id}>
                      {m.manufacturer} / {m.deviceType} / {m.model}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <label>
              ...
              <TextInput
                value={modbusHost}
                onChange={(event) => setModbusHost(event.target.value)}
                placeholder="192.168.1.100 (เว้นว่างถ้าไม่ใช่ Modbus device)"
              />
            </label>
            <label>
              Port
              <TextInput
                ...
                max="65535"
                value={modbusPort}
                onChange={(event) => setModbusPort(event.target.value)}
              />
            </label>
            <label>
              Unit ID
            ...
            <div className="editor-actions full-field">
              ...
              <Button disabled={pending}>
                {pending ? "กำลังบันทึก" : device ? "บันทึก" : "สร้าง Device"}
              </Button>
            </div>
          </form>
        </DialogBody>
...
function PlantEditor({
  ...
  const [organizationId, setOrganizationId] = useState(
    plant?.organizationId ?? defaultOrganizationId ?? "",
  );
  ...
  const [longitude, setLongitude] = useState(
    plant?.longitude?.toString() ?? "",
  );
  const [installedDcKw, setInstalledDcKw] = useState(
    plant?.installedDcKw?.toString() ?? "",
  );
  const [installedAcKw, setInstalledAcKw] = useState(
    plant?.installedAcKw?.toString() ?? "",
  );
  ...
  async function uploadImage(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    const supportedExtension = ["png", "jpg", "jpeg", "webp"].includes(
      extension,
    );
    if (
      ...
      if (!response.ok) {
        ...
        throw new Error(
          detail === "invalid image"
            ? "ไฟล์รูปไม่ใช่ PNG, JPEG หรือ WebP หรือมีขนาดเกิน 2 MiB"
            : "ไม่สามารถอัปโหลดรูปโรงไฟฟ้าได้",
        );
      }
  ...
  return (
    <Dialog
      open
      onOpenChange={(open) => {
        if (!open && !pending && !imagePending) onClose();
      }}
    >
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <div>
            <DialogDescription>Plant registry</DialogDescription>
            <DialogTitle>
              {plant ? "แก้ไขโรงไฟฟ้า" : "เพิ่มโรงไฟฟ้า"}
            </DialogTitle>
          </div>
        </DialogHeader>
        <DialogBody>
          <form className="plant-editor-form" onSubmit={submit}>
            ...
            <label className="full-field">
              ...
              <TextInput
                ...
                maxLength={100}
                placeholder="ไม่กรอก = Asia/Bangkok"
              />
            </label>
            <label>
              Latitude
              <TextInput
                type="text"
                inputMode="decimal"
                placeholder="12.789507 หรือ 12.789507 N"
                value={latitude}
            ...
            <label>
              ...
              <TextInput
                type="text"
                inputMode="decimal"
                placeholder="101.853718 หรือ 101.853718 E"
                value={longitude}
            ...
            <label>
              ...
              <TextInput
                ...
                step="any"
                value={installedDcKw}
                onChange={(event) => setInstalledDcKw(event.target.value)}
              />
            </label>
            <label>
              Installed AC (kW)
              <TextInput
                ...
                step="any"
                value={installedAcKw}
                onChange={(event) => setInstalledAcKw(event.target.value)}
              />
            </label>
            <label>
              Plant Lifecycle
              <Select
                ...
                onValueChange={(value) =>
                  setLifecycleStatus(value as Plant["lifecycleStatus"])
                }
              >
                <SelectTrigger>
                  <SelectValue />
                ...
                <SelectContent>
                  <SelectItem value="IN_CONSTRUCTION">
                    In Construction
                  </SelectItem>
                  <SelectItem value="OPERATIONAL">Operational</SelectItem>
                  <SelectItem value="OFFLINE">Offline</SelectItem>
            ...
            {plant && (
              <div className="plant-image-field full-field">
                ...
                {imagePreview ? (
                  ...
                  <div className="plant-image-fallback">ยังไม่มีรูป</div>
                )}
