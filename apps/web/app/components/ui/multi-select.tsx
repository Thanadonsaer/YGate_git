...
function MultiSelect({
  ...
  const filterBy = [
    "label",
    options.some((option) => option.unit) && "unit",
    options.some((option) => option.tag) && "tag",
  ]
    .filter(Boolean)
    .join(",");
  return (
    <PrimeMultiSelect
      ...
      itemTemplate={(option: MultiSelectOption) => (
        <span className="flex min-w-0 flex-1 items-center gap-2">
          ...
          {option.unit && (
            <span className="shrink-0 rounded bg-canvas px-1.5 py-0.5 text-[11px] font-bold text-ink-soft">
              {option.unit}
            </span>
          )}
          {option.tag && (
            <code className="shrink-0 text-[11px] text-ink-soft">
              {option.tag}
            </code>
          )}
        </span>
      ...
      pt={{
        ...
        token: {
          className:
            "flex items-center gap-1 rounded-full bg-canvas px-2 py-0.5 text-xs font-bold text-ink",
        },
        trigger: {
        ...
        item: (options: {
          context: { focused: boolean; selected: boolean; disabled: boolean };
        }) => ({
          className: cn(
