...
export function usePlatformSession() {
  ...
  if (!value)
    throw new Error("usePlatformSession must be used inside PlatformShell");
  return value;
}
...
export function PlatformShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    applyAccentColor(siteSettings.accentColor);
  ...
  const liveState = useRealtimeSocket(
    undefined,
    (message) => setLastLiveAt(message.sentAt),
    Boolean(user),
  );
  ...
  if (loading) {
    return (
      <main className="loading-screen" aria-label="กำลังโหลด">
        <LivePulse state="connecting" className="loading-pulse" />
        <span>YGATE Solar SCADA</span>
      </main>
    );
  }
          ...
          <div className="brand-lockup">
            <span className="brand-mark">
              {siteSettings.logoUrl ? (
                <img src={assetURL(siteSettings.logoUrl)} alt="" />
              ) : (
                <SunMedium size={19} />
              )}
            </span>
            <div>
              ...
              <small>Solar SCADA · v{BUILD_VERSION}</small>
            </div>
          </div>
          <Button
            variant="bare"
          ...
          <nav aria-label="เมนูหลัก">
            {navigation.map(({ group, items }) => {
              const visible = items.filter(
                (item) => !item.requires || hasPermission(user, item.requires),
              );
