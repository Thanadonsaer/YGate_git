...
func TestLoad(t *testing.T) {
	...
	t.Setenv("AUTH_DATABASE_URL", "postgres://auth@localhost/platform")
	t.Setenv("AUTH_SESSION_IDLE_TIMEOUT", "15m")
	t.Setenv("AUTH_SESSION_ABSOLUTE_TIMEOUT", "8h")
	t.Setenv("AUTH_COOKIE_SECURE", "false")
	t.Setenv("AUTH_PASSWORD_RESET_TTL", "20m")
	cfg, err := Load()
	if err != nil {
		t.Fatal(err)
	}
	if cfg.ListenAddr != "127.0.0.1:44442" || cfg.DatabaseURL != "postgres://auth@localhost/platform?pool_max_conns=10" || cfg.SessionIdleTimeout != 15*time.Minute || cfg.SessionAbsoluteTimeout != 8*time.Hour || cfg.PasswordResetTTL != 20*time.Minute || cfg.CookieSecure {
...
func TestLoadRejectsIdleTimeoutAfterAbsoluteTimeout(t *testing.T) {
	...
	t.Setenv("AUTH_SESSION_ABSOLUTE_TIMEOUT", "1h")
	if _, err := Load(); err == nil {
...
func TestLoadValidatesSMTPRecoveryConfiguration(t *testing.T) {
	...
	t.Setenv("AUTH_SMTP_FROM", "scada@example.com")
	t.Setenv("AUTH_PASSWORD_RESET_URL", "https://scada.example.com/reset-password")
	cfg, err := Load()
	if err != nil {
		t.Fatal(err)
	}
...
func TestLoadAcceptsDatabaseURLAndNormalizesSchemaParameter(t *testing.T) {
	...
	if cfg.ListenAddr != "127.0.0.1:44442" || parsed.Query().Get("schema") != "" || parsed.Query().Get("search_path") != "public" {
		t.Fatalf("config=%+v", cfg)
	}
