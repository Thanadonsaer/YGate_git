...
// platform sends. platform-api (alarm notifications) keeps its own copy of
// the same HTML/CSS -- the two Go modules can't share a package, so this is
// kept in sync by hand across both when the design changes. table-based
// layout + inline styles since mail clients (Outlook especially) don't
// reliably support external/embedded stylesheets or flex.
  ...
  <body style="margin:0;padding:32px 16px;background:#f1f4f6;font-family:-apple-system,'Segoe UI',Roboto,Arial,sans-serif;">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;margin:0 auto;background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #dce3e7;">
      ...
      <tr><td style="padding:28px;">
        ...
        {{if .ButtonURL}}<a href="{{.ButtonURL}}" style="display:inline-block;background:#0e5c73;color:#ffffff;text-decoration:none;font-size:14px;font-weight:700;padding:12px 24px;border-radius:8px;">{{.ButtonLabel}}</a>
        <p style="margin:20px 0 0;font-size:12px;line-height:1.5;color:#8a97a0;">If the button doesn't work, copy this link into your browser:<br><a href="{{.ButtonURL}}" style="color:#0e5c73;word-break:break-all;">{{.ButtonURL}}</a></p>{{end}}
      </td></tr>
...
// ButtonURL+ButtonLabel render a CTA button (with a plain-link fallback)
// when both are set. All fields are HTML-escaped by html/template.
