...
func TestMigrationsAreEmbeddedInOrder(t *testing.T) {
	...
	for _, required := range []string{"token_hash bytea", "csrf_hash bytea", "audit_log_append_only", "organization_id", "Platform Admin", "Organization Admin", "user_role_user_scope_fk", "plant_installed_dc_nonnegative", "raw_register_reading_client_external_unique", "auto_onboard boolean", "'hard_delete', 'user'", "'clear', 'audit'", "'manage_access', 'scada_screen'", "scada_publication_immutable", "DROP CONSTRAINT audit_log_actor_fk", "'delete', 'role'"} {
