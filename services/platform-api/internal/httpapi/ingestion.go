...
func ingestionHandler(service *ingestion.Service) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		result, err := service.Ingest(r.Context(), client, r.Header.Get("Idempotency-Key"), raw, batch, time.Now())
		switch {
		case errors.Is(err, ingestion.ErrInvalidBatch):
			http.Error(w, "invalid telemetry batch", http.StatusBadRequest)
		case errors.Is(err, ingestion.ErrIdempotencyConflict):
			http.Error(w, "idempotency key conflict", http.StatusConflict)
		case err != nil:
			log.Printf("middleware ingestion failed: %v", err)
			http.Error(w, "internal server error", http.StatusInternalServerError)
		default:
