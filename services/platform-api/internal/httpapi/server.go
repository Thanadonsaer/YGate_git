...
func New(version string, ready func(context.Context) error, pool *pgxpool.Pool, sessionIdleTimeout time.Duration, registryService *core.Service, ingestionService *ingestion.Service, hub *gatewayhub.Hub, allowedOrigins ...string) http.Handler {
	...
	if registryService != nil {
		...
		mux.HandleFunc("GET /api/v1/register-profiles", authenticated(pool, sessionIdleTimeout, false, listRegisterProfilesHandler(registryService)))
		mux.HandleFunc("POST /api/v1/register-profiles", authenticated(pool, sessionIdleTimeout, true, createRegisterProfileHandler(registryService)))
		mux.HandleFunc("GET /api/v1/register-profiles/{profileId}/addresses", authenticated(pool, sessionIdleTimeout, false, listRegisterProfileAddressesHandler(registryService)))
		mux.HandleFunc("PUT /api/v1/register-profiles/{profileId}/addresses", authenticated(pool, sessionIdleTimeout, true, upsertRegisterProfileAddressHandler(registryService)))
		mux.HandleFunc("PUT /api/v1/device-models/{modelId}/register-profile", authenticated(pool, sessionIdleTimeout, true, assignRegisterProfileHandler(registryService)))
